...
def get_geneLoad(junction_counts_path, nc, nt, outDir):
    ...
    sample_names = [item.replace('_juncCounts', '') for item in sample_names]
    # creating mapping of gene id to gene name
    gene_df = junction_counts[["gene_id", "gene_name"]]
    gene_mapping = dict(zip(
        gene_df['gene_id'].str.split(',').explode(),
        gene_df['gene_name'].str.split(',').explode()
    ))
    ...
    #adding gene names to end of df
    geneLoad_df['gene_name'] = geneLoad_df['gene_id'].apply(lambda x: gene_mapping.get(x.split(',')[0].strip()))
...
def NMF_iterations(nmf_df):
    ...
    for rank in rank_range:
        nmf = NMF(n_components=rank, max_iter=1000000, init='random')
...
def cluster_geneLoad(geneLoad_path, iterations, processors, outdir, clusters):
...
# Cluster Sample-Load
## master function to select rank, generate rank plot, and cluster the loads
def cluster_sampleLoad(sampleLoad_path, outdir, clusters):
    
    # reading in sample-load data
    sampleLoad = pd.read_csv(sampleLoad_path, sep="\t")
    # sub-setting necessary load columns
    crypticLoad_cols = [col for col in sampleLoad.columns if col.endswith('crypticLoad')]
    sampleLoad_data = sampleLoad[crypticLoad_cols]
    sampleLoad_values = np.array(sampleLoad_data).reshape(-1,1)
    ...
    if clusters:
        ...
        for k in range(2,len(sampleLoad_values)):
            kmeans = KMeans(n_clusters=k, random_state=0)
            kmeans.fit(sampleLoad_values)
            # Silhouette score
            silhouette = silhouette_score(sampleLoad_values, kmeans.labels_)
            silhouette_scores.append(silhouette)
            # Davies-Bouldin index
            davies_bouldin = davies_bouldin_score(sampleLoad_values, kmeans.labels_)
            davies_bouldin_scores.append(davies_bouldin)
            # Calinski-Harabasz index
            calinski_harabasz = calinski_harabasz_score(sampleLoad_values, kmeans.labels_)
            calinski_harabasz_scores.append(calinski_harabasz)
            # Reconstruction error (inertia)
            reconstruction_error = kmeans.inertia_
            reconstruction_errors.append(reconstruction_error)
        # plotting the metrics
        plot_rank_metrics(silhouette_scores, davies_bouldin_scores, calinski_harabasz_scores, reconstruction_errors, outdir)
        # automating rank selection by selecting the highest of silhouette, davies-bouldin, and calinski-harabasz (if not given by user)
        max_index_silhouette = np.argmax(silhouette_scores)+2
        min_index_davies_bouldin = np.argmin(davies_bouldin_scores)+2
        max_index_calinski_harabasz = np.argmax(calinski_harabasz_scores)+2
        indices = [max_index_silhouette, min_index_davies_bouldin, max_index_calinski_harabasz]
        rank = max(set(indices), key=lambda x: indices.count(x))
    # running the k-means clustering algorithm with inferred rank or user-provided rank
    kmeans = KMeans(n_clusters=rank, random_state=0)
    kmeans.fit(sampleLoad_values)
    # setting up sample-load clutering dataframe
    cluster_labels = kmeans.labels_
    sampNames=sampleLoad_data.columns 
    distances = kmeans.transform(sampleLoad_values)
    silhouette_scores = silhouette_samples(sampleLoad_values, kmeans.labels_)
    sampleLoad_clusters = pd.DataFrame(columns=["samples", "cluster", "silhouette"])
    sampleLoad_clusters["samples"]=sampNames
    sampleLoad_clusters["cluster"]=cluster_labels
    sampleLoad_clusters["silhouette"]=silhouette_scores
    # save sample cluster membership
    if sampleLoad_clusters.empty:
        return(0)
    else:
        # save entire H matrix to better be able to interpret clusters for now  
        sampleLoad_clusters.to_csv(outdir+"Sample_ClusterMembership.txt", sep="\t", index=False)
        return(1)
